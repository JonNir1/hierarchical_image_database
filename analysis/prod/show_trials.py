...
Run from repo root:
    # all participants
    .venv/Scripts/python analysis/prod/show_trials.py

    # filter by task version
    .venv/Scripts/python analysis/prod/show_trials.py --version 3.06

    # single participant -- by participant_id (exact or substring match)
    .venv/Scripts/python analysis/prod/show_trials.py --participant 67111bf261090940b6316e55
    .venv/Scripts/python analysis/prod/show_trials.py -p 67111bf2

    # single participant -- pass any substring of the session filename
    .venv/Scripts/python analysis/prod/show_trials.py 18h36.03

    # combine filters (version + participant + filename substring all AND together)
    .venv/Scripts/python analysis/prod/show_trials.py --version 3.06 -p 67111bf2

    # list available session filenames (with versions and participant_ids) without
    # opening anything
    .venv/Scripts/python analysis/prod/show_trials.py --list
"""
import sys
import warnings
...
while i < len(args):
    ...
    if arg == "--version":
        if i + 1 >= len(args):
            print("Error: --version requires a value (e.g. --version 3.06)")
            sys.exit(1)
        try:
            version_filter = float(args[i + 1])
        except ValueError:
            print(f"Error: invalid version '{args[i + 1]}' — must be a number (e.g. 3.06)")
            sys.exit(1)
        i += 2
    elif arg in ("--participant", "-p"):
        if i + 1 >= len(args):
            print(f"Error: {arg} requires a value (e.g. {arg} 67111bf2)")
            sys.exit(1)
...
if version_filter is not None:
    ...
    if not filtered:
        print(
            f"No participants found for task version {version_filter:g}.\n"
