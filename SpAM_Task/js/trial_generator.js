 ...
 * This guarantees no catch trial lands at index 0 or the final index.
 * Example: numMain=10, numCatch=2 → positions [3, 7].
 *
 * @param {string[][]} mainTrials  - Output of buildTrialLists
 * @param {string[]}   catchImages - Fixed image set used for every catch trial
 * @param {number}     numCatch    - Number of catch trials to insert
 * @returns {Array<{type: 'main'|'catch', images: string[]}>}
 */
function insertCatchTrials(mainTrials, catchImages, numCatch) {
    const numMain = mainTrials.length;

    // Compute combined-array positions where catch trials will be placed.
    // Using an array (ascending order) lets us advance a single pointer while
    // iterating, avoiding repeated set-membership checks.
    ...
    for (let pos = 0; pos < numMain + numCatch; pos++) {
        if (catchIdx < catchPositions.length && pos === catchPositions[catchIdx]) {
            combined.push({ type: 'catch', images: catchImages });
            catchIdx++;
        } else {
