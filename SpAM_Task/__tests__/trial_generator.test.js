    ...
    it('throws when the image pool cannot fill all trials', () => {
        ...
        assert.throws(() =>
            buildTrialLists(['x.png','y.png'],
                { design: { trials_per_subject:3, images_per_trial:2, frac_trials_repeated:0 } },
                makeRng()),
...
describe('insertTrialRepeats', () => {
    it('returns trials unchanged (wrapped) when frac_trials_repeated is 0', () => {
        const trials = buildTrialLists(ALL_IMAGES, CONFIG, makeRng());
        const result = insertTrialRepeats(trials, CONFIG, makeRng());
    ...
    it('produces exactly n_trial_repeats repeat entries with matching image sets', () => {
        const cfg = { design: { ...CONFIG.design, frac_trials_repeated: 0.3 } }; // t_distinct=7, numRepeats=3
        const trials = buildTrialLists(ALL_IMAGES, cfg, makeRng());
        const result = insertTrialRepeats(trials, cfg, makeRng());
    ...
    it('respects min_trial_repeat_separation', () => {
        const cfg = { design: { ...CONFIG.design, frac_trials_repeated: 0.3, min_trial_repeat_separation: 3 } };
        const trials = buildTrialLists(ALL_IMAGES, cfg, makeRng());
        const result = insertTrialRepeats(trials, cfg, makeRng());
    ...
    it('is deterministic with the same seed', () => {
        const cfg = { design: { ...CONFIG.design, frac_trials_repeated: 0.3 } };
        const run = seed => {
            const rng = makeRng(seed);
            const trials = buildTrialLists(ALL_IMAGES, cfg, rng);
            return insertTrialRepeats(trials, cfg, rng);
        };
    ...
    it('throws when min_trial_repeat_separation cannot be satisfied', () => {
        const cfg = {
            design: {
                ...
                frac_trials_repeated: 0.5, // t_distinct=2, numRepeats=2
                min_trial_repeat_separation: 3, // impossible to fit 2 repeats with gap 3 in 4 slots
            },
        };
        const trials = buildTrialLists(ALL_IMAGES, cfg, makeRng());
        assert.throws(
            () => insertTrialRepeats(trials, cfg, makeRng()),
