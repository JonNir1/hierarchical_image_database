...
class TestMainErrors:
    ...
    def test_empty_stimuli_path_prints_warning(self, task_env, capsys):
        make_config(task_env)  # all paths empty
        gm.main()
        assert "WARNING" in capsys.readouterr().out

    def test_nonexistent_stimuli_dir_prints_warning(self, task_env, capsys):
        make_config(task_env, stimuli_path=str(task_env / "nonexistent"))
        gm.main()
        assert "WARNING" in capsys.readouterr().out

    def test_nonexistent_stimuli_dir_omitted_from_manifest(self, task_env):
        make_config(task_env, stimuli_path=str(task_env / "nonexistent"))
        gm.main()
...
class TestMainManifestContent:
    def test_all_three_sets_written_when_paths_valid(self, task_env):
        for name, key in (("stimuli", "stimuli_path"),
        ...
        make_config(
            task_env,
            stimuli_path=str(task_env / "stimuli"),
    ...
    def test_image_filenames_stored_in_manifest(self, task_env):
        stimuli_dir = task_env / "stimuli"
        stimuli_dir.mkdir()
        (stimuli_dir / "cat.png").touch()
        (stimuli_dir / "dog.png").touch()
        make_config(task_env, stimuli_path=str(stimuli_dir))
        gm.main()
        manifest = json.loads((task_env / "stimuli_manifest.json").read_text())
    ...
    def test_empty_stimuli_dir_produces_empty_list(self, task_env):
        stimuli_dir = task_env / "stimuli"
        stimuli_dir.mkdir()
        make_config(task_env, stimuli_path=str(stimuli_dir))
        gm.main()
...
class TestMainValidation:
    def test_warns_when_images_set_is_empty(self, task_env, capsys):
        stimuli_dir = task_env / "stimuli"
        stimuli_dir.mkdir()
        make_config(task_env, stimuli_path=str(stimuli_dir))
        gm.main()
    ...
    def test_warns_when_too_few_practice_images(self, task_env, capsys):
        ...
        make_config(task_env, stimuli_practice_path=str(practice_dir),
    ...
    def test_warns_when_too_few_catch_images(self, task_env, capsys):
        ...
        make_config(task_env, stimuli_catch_path=str(catch_dir),
                    images_per_trial=20)
        gm.main()
        out = capsys.readouterr().out
        assert "WARNING" in out
