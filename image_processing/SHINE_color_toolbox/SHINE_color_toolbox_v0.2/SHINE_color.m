...
function images = SHINE_color(images,templ,varargin)
...
for i=1:length(varargin)
    if i==1;input_folder = varargin{1};
    elseif i==2;output_folder = varargin{2};
    elseif i==3;template_folder = varargin{3};
    else;error('only three optional inputs allowed')
    end
end
...
for im = 1:numim
    if nargout == 0
        if nargin > 0
            ...
            imwrite(color_im,fullfile(output_folder,imname{im}),'Alpha',templ{im}); % SHINE_color: writing the colorful image
        else
            % SHINE_color: rescale value channel from 0-255 to 0-1
