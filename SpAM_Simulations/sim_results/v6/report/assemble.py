"""Build the shareable v6 report: hand-written prose plus computed figures.

    python SpAM_Simulations/sim_results/v6/report/assemble.py --run <path to the v6 run dir>

``report_v6.src.html`` holds the prose and is edited by hand. Wherever it contains
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument("--run", type=Path, default=HERE.parent,
                   help="the v6 run directory holding out/, mds_store/, gt/ and gt_diagnostics/. "
                        "Defaults to this file's parent, which is right once the report and the "
                        "data live in the same checkout.")
    p.add_argument("--src", type=Path, default=HERE / "report_v6.src.html")
    p.add_argument("--out", type=Path, default=HERE / "report_v6.html")
    p.add_argument("--title", default="SpAM Simulations v6 - the decision run")
    args = p.parse_args(argv)

    # A WARNING, not an abort, unlike v5's. v6's figures render an explicit "not produced yet"
    # placeholder for a missing table, precisely so the prose can be written and reviewed while the
    # run is still going. Aborting here would make that impossible; inventing data would be worse
    # than either. The placeholders are loud enough that a half-built page cannot be mistaken for a
    # finished one.
    if not (args.run / "out").is_dir():
        print(
            f"[assemble] WARNING: no out/ under {args.run}; data figures render as placeholders.\n"
            f"  The run data is gitignored and may sit in another checkout; pass it explicitly:\n"
            f"    --run <repo>/SpAM_Simulations/sim_results/v6")
