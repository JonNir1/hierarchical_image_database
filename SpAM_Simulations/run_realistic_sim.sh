#!/usr/bin/env bash
#
# Provision an EC2 instance, run the full SpAM MDS sweep for the REALISTIC simulation - each
...
# the results to S3.
#
# For the original, uniform-random simulation, see the sibling run_uniform_sim.sh. Both source
...
sim = pipeline.generate_realistic_simulation(cfg, verbose=True)
