...
**The empirical sample is the screening block's own reliability, for all 84 candidates.** Two
reasons, and both matter. The 67 retained subjects are a *truncated* sample of the population the
gate operates on, so fitting on them alone understates the lower tail - exactly the part that
determines what screening yields. And every candidate has exactly two screening repeats, whereas
`subject_reliability_sample` would average two repeats for the screened-out and four for the
...
def _observed(data_dir: str, manifest: str, config: str):
    ...
    # Homogeneous across all 84: two screening repeats each. See the module docstring.
    reliability = attempted["median_reliability"].astype(float).dropna().to_numpy()
...
def _simulate_cell(coords, cal, threshold, *, n_subjects, reps, seed, simulator):
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"  screening-block reliability: n={len(reliability)} "
